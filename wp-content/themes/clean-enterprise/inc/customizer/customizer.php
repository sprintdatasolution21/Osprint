 ...
 */
function clean_enterprise_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'clean_enterprise_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'clean_enterprise_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'clean_enterprise_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'clean-enterprise' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	clean_enterprise_register_option( $wp_customize, array(
			'name'              => 'clean_enterprise_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Clean_Enterprise_Important_Links',
			'label'             => esc_html__( 'Important Links', 'clean-enterprise' ),
			'section'           => 'clean_enterprise_important_links',
			'type'              => 'clean_enterprise_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'clean_enterprise_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function clean_enterprise_customize_preview_js() {
	wp_enqueue_script( 'clean-enterprise-customize-preview', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'assets/js/customizer.min.js', array( 'customize-preview' ), '20170816', true );
}
add_action( 'customize_preview_init', 'clean_enterprise_customize_preview_js' );
